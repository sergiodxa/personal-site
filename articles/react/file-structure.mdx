...
- The component represents a whole feature (usualy this is realted to the point above)
...
And then call the hook and get the current logged-in user, because SWR dedupes requests I don't need to care about using fetching many times because I call it many times, this let me avoid using Context.
...
The files here must always have a single `export default`, this is required to be able to lazily import the routes in the entry point which is where I define my pages.
