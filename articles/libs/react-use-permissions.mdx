...
> Valid names are `accelerometer`, `accessibility-events`,
> `ambient-light-sensor`, `background-sync`, `camera`​, `clipboard-read`,
> `clipboard-write`, `geolocation`, `gyroscope`, `magnetometer`, `microphone`,
> `midi`, `notifications`, `payment-handler`, `persistent-storage`, and `push`
...
When the component is rendered the hook will return `null` initially and then
check if the user has already granted or denied the permissions returning a
boolean. In case the user hasn't already it will be prompted and then the hook
