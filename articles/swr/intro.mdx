...
SWR is an all new data fetching library made by the team at [ZEIT](https://zeit.co), the same team who created Now and [Next.js](https://nextjs.org/).

This new library is made specifically for React applications as a custom hook with a super simple interface. Let's create a super simple project using SWR and Next.js.
...
And that's all, if we our our project with `yarn dev` we already have a page with some components, but we could delete them.
...
Our `fetcher` above is a simple module which will run the `fetch` and reply with the JSON data instead of the HTTP response.
...
With this we have a list of the first 20 Pokémon names, let's create a new component to show more information of each one.
...
Now in our HomePage we need to import and use `PokemonShort`.
...
Now we should see a list of card with the data of each Pokémon.

![Our Next.js + SWR Pokedex Application](/static/articles/swr/intro.png "Our Next.js + SWR Pokedex Application")

With this we saw we to use SWR to fetch a list of data and then render a component per item and fetch more data there.

In future articles I will continue working on this application and add more feature.
