...
In previous articles we built a Pokedex project [using SWR](/articles/swr/intro/) and then we added it [pagination](/articles/swr/pagination/) with the same library. Today we will learn how we can use Suspense to handle the loading states while we fetch the data, and without using the experimental version of React.js.
