...
But the key thing here is that we, as developer, may probably know which data the user will need on each page, or at least most of it, so it's possible to fetch it before the user trigger a navigation.
...
This cache it's also updatable from the outside using a simple function called `mutate` which SWR give us. This is great since we could call this function and then once a React component is rendered using SWR it will already have the data in cache.
...
Now if the user clicks on **Home** we now we will not show any dynamic data, so we don't care about that link, we could let Next.js prefetch the JS bundle and call it a day.
...
Now if the user mouse enter the link, aka the user hover the link, we will call our `prepare` function, we could do this because if the user hover the link it may want to click it, so we trigger the fetch of the data, once the user click it it may have already fetched it and updated SWR cache, if the user never click we only prefetched data and cache it for nothing but didn't lose anything.
...
The `fetcher` function trigger the `fetch` and parse de response as JSON.

The `fetchAndCache` function will call `fetcher`, keep the promise, not the result since we are not awaiting it or calling `.then`, and pass the `key`, our URL, to `mutate` along the request promise, the `false` as third argument will tell SWR to not revalidate the data against the backend, we don't need it because we just fetched it so we will it to don't do that.
