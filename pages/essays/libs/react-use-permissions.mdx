...
Then use it inside a any functional React component passing any valid name.
...
When the component is rendered it will return `null` initially and then check if the user has already granted or denied the permissions returning a boolean. In case the user hasn't already it will be prompted and then the hook will return the new state.
