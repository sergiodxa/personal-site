...
export const meta = {
  ...
  slug: "composicion-componentes-react",
  lang: "es",
...
Hasta ahora [hemos creado un solo componente](/essays/componentes-con-react), lo más normal al usar React es tener múltiples componentes y combinarlos para formar un componente más complejo. Hoy vamos a ver como hacer esto.

Si [recordamos del artículo anterior](/essays/tipos-datos-react) nuestro componente debe lucir así:
...
Si vemos acabamos de crear nuestro nuevo componente que [crea un elemento](/essays/hola-mundo-react) `<header>` y dentro de este crea otro elemento usando el componente `Hello`, a parte de esto hicimos que este nuevo componente reciba el prop `name` y se lo pase tal cual a `Hello`.
...
¡Con esto acabas de crear tu segundo componente! [En otro artíuclo veremos como mejorar Header para personalizar el contenido dentro de `<header>`](/essays/componentes-react-hijos-personalizables).
