...
Hooks es una **propuesta de un nuevo feature** para React que está disponible para usar ya mismo instalando `react@next` y `react-dom@next` o `@v16.7.0-alpha` que es la versión exacta y en caso de ser aceptada va a ser liberada en la versión `16.7`.

> Documentación oficial: https://reactjs.org/docs/hooks-intro.html
> RFC: https://github.com/reactjs/rfcs/pull/68

## Charla en la React Conf 2018

<Video src="https://www.youtube.com/embed/dpw9EHDh2bM" />
...
Este hook es similar a `useState` con la diferencia de que funciona similar a Redux, el primer argument es un reducer que recibe `state` y `action` y el segundo es el estado inicial.
...
Este hook recibe un tercer argumento que es una acción que va a ser automaticamente despachada antes de iniciar el reducer, esto nos permite inicializar el estado con valores derivados de los props del componente.
...
Con eso cuando se inicie el estado su count va a ser igual a initialCount.

### `useCallback`

Este método nos permite obtener una versión memoizada de un callback que solo va a ser regenerada si ciertos argumentos indicados al hook cambian. Esto es útil cuando necesitamos pasar una función a un componente hijo para que este actualice el estado de su padre, de esta forma el componente no va a tener problemas de doble renders.
...
Sin usar `useCallback` cada vez que se vuelva a renderizar `Counter` se generaría una nueva función `handleClick` lo que forzaría que se vuelva a renderizar `Button` innecesariamente, gracias a este hook solo se va a generar un nuevo `handleClick` cuando `setCount` cambie, lo que no debería pasar nunca. 

Así el HOC `memo` que agregamos a `Button` puede correctamente validar si `onClick` o `children` cambian.

### `useMemo`

Este hook nos permite memoizar un valor generado como resultado de una función, ideal para cuando necesitamos ejecutar una función pesada y necesitamos solo volver a ejecutarla si ciertos valores cambiaron.

Digamos que tenemos un string, muy grande, que nos llega como `props` y necesitamos convertirlo en un objeto usando `JSON.parse`, dependiendo del tamaño del JSON esto puede tardar por lo que vamos a memoizarlo.
...
function List({ raw }) {
  ...
  return (
    <>
      {data.map(datum => <Item key={datum.id} {...datum} />)}
    </>
  )
}
```

Con esto solo volveríamos a parsear `raw` cuando este cambia. De la misma forma se podrían optimizar para muchos otros casos, aunque esto es especialmente útil cuando tenemos uno de muchos props que necesita usarse para algún cálculo pesado mientras que el resto no, de esta forma evitamos el cálculo si los demás props no cambiaron.

### `useRef`

Este hook nos devuelve un valor modificable cuya propiedad `.current` es igual al valor inicial que le pasamos al hook.

Este `ref` nos permite tanto obtener elementos del DOM para usarlos luego como guardar cualquier valor que necesitemos mantener entre renders.
...
Con esto `$input.current` va a ser igual al elemento del DOM de nuestro input y podemos usarlo para ejecutar métodos como `.focus()`.

### `useImperativeMethods`

Este hook nos permite definir métodos que el padre de nuestro componente podría usar mediante `refs` para interactuar con nuestro componente o con sus hijos.

```js
import React, { forwardRef, useImperativeMethods } from "react";


const FocusInput = forwardRef(FocusInput() {
  const $input = useRef(null);
