...
function matchElement({ tagName, children, attributes }, index) {
  ...
  switch (tagName) {
    ...
    case 'pre': {
      const { value: className } = (children[0].attributes || []).find(
        ({ key }) => (console.log(key), key === 'class')
      );
      if (className && className.indexOf('request') >= 0) {
        const { content } = children[0].children[0];
        const data = safeLoad(entities.decode(content));
        return createElement(Request, { ...data, ...baseProps });
      }
