...
// custom plugin for CodeSandbox
const codesandbox = regexp(/@\[codesandbox\]\(([^\)]*)\)/, (match, utils) => {
  const id = match[1];
  const url = `https://codesandbox.io/embed/${id}`;

  return `<iframe src="${url}" style="width:100%; height:500px; border:0; border-radius: 4px; overflow:hidden;" sandbox="allow-modals allow-forms allow-popups allow-scripts allow-same-origin"></iframe>`
});
